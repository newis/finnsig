...
def generate_hmac(key, method, path, query_string="", service=""):
    """Calculate the FINN-GW-KEY using HMAC-SHA512."""
    method = method.upper()
...
def finn_hmac(method, path, query_string, service_header):
    """Generate the FINN-GW-KEY for a given HTTP method, path, query string, and service header."""
    encoded_hmac_key = 'MQD1MzLjZ2ZgLwp4Zl00ATD5YJV5ATRgLzVlBTEvLmNkAzR2'
    decoded_hmac_key = decode_hmac_key(encoded_hmac_key)
    return generate_hmac(decoded_hmac_key, method, path, query_string, service_header)
